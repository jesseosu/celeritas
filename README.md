...
### Cancel Order
1. Producer generates `CancelRequest`
2. If found → remove from book, emit `CancelAck`, publish market data update
3. If not found → emit `Reject`
...
| `test_price_time_priority` | FIFO ordering at identical price levels |
| `test_spsc_smoke` | Lock-free queue correctness under load |
| `test_orderbook_l2` | Order book state correctness |
| `test_proto` | Protocol encoding/decoding |
